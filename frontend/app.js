...
function showRecord(record, target = "query-result", transactionHash = "") {
  ...
  const technical = document.createElement("details"); technical.className = "technical-result"; const summary = document.createElement("summary"); text(summary, "Technical result"); technical.append(summary); const list = document.createElement("dl"); list.className = "technical-list"; [["wallet", short(record.wallet)], ["authenticated caller", short(record.authenticated_caller || record.evaluated_by)], ["authorization mode", record.authorization_mode || "self"], ["raw status", record.status || "—"], ["score band", record.score_band || "—"], ["risk", record.risk || "—"], ["evidence source", record.evidence_provenance?.source || "genlayer_studionet_rpc"], ["provenance status", record.evidence_provenance?.retrieval_status || "verified"], ["evidence hash", record.evidence_hash || "—"], ["schema version", record.evidence_schema_version || "—"], ["transaction hash", transactionHash ? short(transactionHash) : "—"], ["raw reasoning", record.reasoning || "—"]].forEach(([key, value]) => { const dt = document.createElement("dt"); text(dt, key); const dd = document.createElement("dd"); text(dd, value); list.append(dt, dd); }); technical.append(list); el.append(technical);
